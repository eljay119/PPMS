                        ...
                        <tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-warning" data-bs-toggle="modal"
                                    data-bs-target="#certifyModal{{ $project->id }}">Certify</button>
                            </td>
                        </tr>

                        <!-- Modal -->
                        <div class="modal fade" id="certifyModal{{ $project->id }}" tabindex="-1"
                            aria-labelledby="certifyModalLabel{{ $project->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="certifyModalLabel{{ $project->id }}">Certify Project
                                        </h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        Are you sure you want to certify the project
                                        <strong>{{ $project->title }}</strong>?
                                    </div>
                                    <div class="modal-footer">
                                        <form method="POST" action="{{ route('budget_officer.certify') }}">
                                            @csrf
