...
@section('content')
                            ...
                            <tbody>
                                @forelse($ppmp->ppmpProjects as $project)
                                    <tr>
                                        ...
                                        <td>{{ $project->appProject->status->name ?? '' }}</td>
                                        <td>{{ $project->appProject->title ?? '' }}</td>
                                        <td>{{ $project->appProject->endUser->name ?? '' }}</td>
                                        <td>
                                            <buttona type="button" class="text-warning me-2" title="Edit"
                                                ...
                                                <button type="button"
                                                    ...
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#deleteProjectModal{{ $project->id }}" title="Delete">
    ...
    @foreach ($ppmp->ppmpProjects as $project)
        ...
        <div class="modal fade" id="editProjectModal{{ $project->id }}" tabindex="-1"
            ...
            <div class="modal-dialog">
                <form action="{{ route('head.ppmp_projects.update', $project->id) }}" method="POST"
                    class="modal-content">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="ppmp_id" value="{{ $ppmp->id }}">
                    <div class="modal-header">
                        ...
                        <buttona type="button" class="text-warning me-2" data-bs-dismiss="modal" aria-label="Close">
                            </button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="amount" class="form-label">Amount</label>
                            <input type="number" class="form-control" name="amount" value="{{ $project->amount }}"
                                required>
                        </div>
                        <div class="mb-3">
                            <label for="category" class="form-label">Category</label>
                            ...
                            <select class="form-select" name="mode_of_procurement_id" required>
                                @foreach ($modes as $mode)
                                    <option value="{{ $mode->id }}"
                                        {{ $project->mode_of_procurement_id == $mode->id ? 'selected' : '' }}>
                                        {{ $mode->name }}
                                    </option>
                                @endforeach
                            </select>
        ...
        <div class="modal fade" id="deleteProjectModal{{ $project->id }}" tabindex="-1"
            ...
            <div class="modal-dialog">
                <form action="{{ route('head.ppmp_projects.destroy', $project->id) }}" method="POST"
                    class="modal-content">
                    @csrf
                    @method('DELETE')
                    <div class="modal-header">
    ...
    $('.editProjectBtn').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#editProjectModal #mode_of_procurement_id').val(response.project
...
@if (session('success'))
    <div class="toast-container position-fixed bottom-0 end-0 p-3" style="z-index: 1055;">
        <div class="toast align-items-center text-white bg-success border-0" id="sessionToast" role="alert"
...
@elseif (session('error'))
    <div class="toast-container position-fixed bottom-0 end-0 p-3" style="z-index: 1055;">
        <div class="toast align-items-center text-white bg-danger border-0" id="sessionToast" role="alert"
            aria-live="assertive" aria-atomic="true">
            <div class="d-flex">
                <div class="toast-body">
                    {{ session('error') }}
